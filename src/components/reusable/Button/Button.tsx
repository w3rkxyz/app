...
const MyButton: React.FC<ButtonElement> = ({ buttonText, buttonType, buttonStyles, action }) => {
  const getButtonTypes = (type: string) => {
    switch (type) {
      ...
      case "dropdown":
        return "text-[10px] sm:text-[10px] font-semibold font-secondary leading-[20px] sm:leading-[12px] tracking-[-1%] w-full sm:w-auto md:w-auto h-[24px] sm:h-[31px] sm:py-[10px] sm:px-3 md:px-5 text-center  rounded-[4px] text-[#000000]";
      default:
        return "bg-gray-500";
    }
  };
